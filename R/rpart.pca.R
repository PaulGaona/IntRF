...
"rpart.pca" <-
  function(
      tree, pts = TRUE, plt.allx = TRUE, speclabs = TRUE,
    ...
    old.par <- graphics::par(mar = rep(2, 4), xpd = TRUE)
    ...
    if (plt.allx) {
      ...
      mxx <- sqrt(apply(x[, c(cv1, cv2)]^2, 1, sum))
    }
    cvar <- round((100 * z$d[1:maxd]^2) / sum(z$d[1:maxd]^2), digits = 2)
    cvar2 <- round(
      diag(stats::cor(xall, xx[order(tree$where), ]))[1:maxd],
      3
    )
    dlabs <- paste(
      "   Dim ", c(1:maxd), " ", cvar, "% : [",
    ...
    repeat {
      ...
      sze <- graphics::par()$fin / graphics::par()$din
      adj <- ifelse(pts, cxy[2] * sze[2], 0)
      if (specvecs) {
        graphics::segments(sc * y[, cv1], sc * y[, cv2], rep(0, nrow(y)),
          rep(0, nrow(y)),
          col = "gray", lty = 1
        )
      }
      graphics::mtext(dlabs[cv1],
        side = 1, las = 0, adj = 0, line = 0,
        cex = 0.85 * graphics::par()$cex
      )
      graphics::mtext(dlabs[cv2],
        side = 2, las = 0, adj = 0, line = 0,
        cex = 0.85 * graphics::par()$cex
      )
      if (add.tree) {
        ...
        to <- pp[(nn + 1):(2 * nn)]
        graphics::segments(
          x[from, cv1], x[from, cv2], x[to, cv1],
          x[to, cv2]
        )
      }
      if (chulls) {
        ...
        for (i in 1:length(unitg)) {
          hpts <- grDevices::chull(xx[unitg[i] == treegrps, c(cv1, cv2)])
          hpts <- c(hpts, hpts[1])
          graphics::lines(xx[unitg[i] == treegrps, c(cv1, cv2)][hpts, ], col = i + 1)
        }
      }
      if (plt.allx) {
        unitg <- sort(unique(treegrps))
        for (i in 1:length(unitg)) {
          graphics::points(xx[unitg[i] == treegrps, cv1], xx[unitg[i] == treegrps, cv2],
            pch = 21, col = 1, bg = i + 1, cex = 1.2 * graphics::par()$cex
          )
        }
      }
      if (pts) {
        ...
        for (i in 1:length(lvnode)) {
          graphics::points(xlv[, cv1][lvnode[i] == lvnode], xlv[, cv2][lvnode[i] == lvnode],
            pch = 21, cex = 2 * graphics::par()$cex, col = 1, bg = i + 1
          )
        }
      }

      if (speclabs) {
        graphics::text(sc * y[, cv1], sc * y[, cv2] + 0.5 * adj * specvecs *
          (y[, cv2] > 0), specs, col = "black", cex = graphics::par()$cex)
      }
      graphics::points(0, 0, pch = 3, cex = graphics::par()$cex * 2.5, col = 1)
      if (interact) {
        z <- graphics::locator(1)
        if (length(z$x)) {
          if (z$x > 0 & z$y < 0) {
            if (cv1 < maxd) {
              cv1 <- cv1 + 1
