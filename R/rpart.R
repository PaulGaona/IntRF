...
"rpart" <-
  function(
      formula, data = NULL, weights, subset, na.action = na.rpart,
    ...
    wt <- stats::model.extract(m, "weights")
    if (length(wt) == 0) {
      wt <- rep(1, nrow(m))
    }
    ...
    cats <- rep(0, ncol(X))
    if (!is.null(xlevels)) {
      cats[match(names(xlevels), dimnames(X)[[2]])] <- unlist(lapply(
        xlevels,
        length
      ))
    }
    ...
    if (length(extraArgs)) {
      ...
      if (any(indx == 0)) {
        stop(paste(
          "Argument", names(extraArgs)[indx == 0],
          "not matched"
        ))
      }
    }
    ...
    rpfit <- .C("s_to_rp",
      n = as.integer(nobs), nvarx = as.integer(nvar),
    ...
    rp <- .C("s_to_rp2", as.integer(nobs), as.integer(nsplit),
      ...
      which = integer(nobs), cptable = matrix(double(numcp *
        cpcol), nrow = cpcol), dsplit = matrix(
        double(1),
        nsplit, 3
      ), isplit = matrix(integer(1), nsplit, 3),
      csplit = catmat, dnode = matrix(double(1), nodes, 3 +
        numresp), inode = matrix(integer(1), nodes, 6), PACKAGE = "IntRF"
    )
    ...
    splits <- matrix(c(rp$isplit[, 2:3], rp$dsplit),
      ncol = 5,
      dimnames = list(dn1, c(
        "count", "ncat", "improve", "index",
        "adj"
      ))
    )
    index <- rp$inode[, 2]
    ...
    if (nsplit == 0) {
      frame <- data.frame(row.names = 1, var = "<leaf>", n = rp$inode[
        ,
        5
      ], wt = rp$dnode[, 3], dev = rp$dnode[, 1], yval = rp$dnode[
        ,
        4
      ], complexity = rp$dnode[, 2], ncompete = pmax(
        0,
        rp$inode[, 3] - 1
      ...
      frame <- data.frame(row.names = rp$inode[, 1], var = factor(
        svar,
        0:ncol(X), tname
      ), n = rp$inode[, 5], wt = rp$dnode[
        ,
        3
      ], dev = rp$dnode[, 1], yval = rp$dnode[, 4], complexity = rp$dnode[
        ,
        2
      ], ncompete = pmax(0, rp$inode[, 3] - 1), nsurrogate = rp$inode[, 4])
    }
    if (method == "class") {
    ...
    if (nsplit == 0) {
      ans <- list(
        frame = frame, where = where, call = call,
