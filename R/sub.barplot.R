...
"sub.barplot" <-
  function(
      x, y, z, keep = rep(TRUE, length(x)), row.scale = FALSE, xadj = 1,
    ...
    drawbar <- function(x, y, z, line.adj = 0, xwid, ywid, border = TRUE,
      ...
      for (i in 1:nbar) {
        ...
        yb <- y - ywid + z[i] * ywid * c(0, 0, 1, 1) - line.adj *
          ywid
        graphics::polygon(xb, yb, col = colbar[i])
      }
      if (border) {
        graphics::polygon(xx, yy, col = 1, density = 0)
      }
      if (line) {
        graphics::lines(xx[1:2], yy[1:2] - line.adj * ywid)
      }
    }
