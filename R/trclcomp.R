...
"trclcomp" <- function(x, method = "com", km = TRUE, mrt = TRUE) {
  ...
  hclout <- stats::hclust(d, method = method)
  grp.mrt <- pruner(x)
  for (i in 2:nr) {
    grp.clust <- factor(stats::cutree(hclout, k = size[i]))
    cents <- t(sapply(split(as.data.frame(x$y), grp.clust), colMeans))
    grp.clust <- factor(stats::kmeans(x$y, centers = cents)$cluster)
    cents.mrt <- t(sapply(split(as.data.frame(x$y), grp.mrt[, i]), colMeans))
    grp.mrt.clust <- factor(stats::kmeans(x$y, centers = cents.mrt)$cluster)
    clust.err[i] <- sum(stats::resid(stats::lm(x$y ~ factor(grp.clust), singular.ok = TRUE))^2) / sst
    mrt.clust.err[i] <- sum(stats::resid(stats::lm(x$y ~ factor(grp.mrt.clust), singular.ok = TRUE))^2) / sst
  }
  minerr <- min(c(mrt.err, mrt.clust.err, clust.err))
  plot(size, mrt.err,
    type = "n", ylim = c(minerr, 1), xlab = "Size",
    ylab = "Resubstition Error"
  )
  graphics::points(size, mrt.err, type = "o", col = 2, pch = 16)
  graphics::points(size, mrt.clust.err, type = "o", col = 3, pch = 16)
  graphics::points(size, clust.err, type = "o", col = 4, pch = 16)
  graphics::legend(mean(size), 1, c("MRT", "MRT-Cluster", "Cluster"),
    col = c(2:4),
    lty = 1, bty = "n"
  )
  graphics::title("Comparison of tree and cluster errors across size")
