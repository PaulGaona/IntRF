...
"snip.rpart.mouse" <- function(tree, parms) {
  # ~     parms=paste(".rpart.parms", grDevices::dev.cur(), sep = ".")) {
  ...
  lastchoice <- 0
  while (length(choose <- graphics::identify(xy, n = 1, plot = FALSE)) > 0) {
    ...
    if (choose != lastchoice) {
      ...
      graphics::lines(c(draw$x[, temp]), c(draw$y[, temp]), col = 0)
