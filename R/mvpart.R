#' @export
"mvpart" <- function(
    form, data, minauto = TRUE, size, xv = c(
      "1se", "min",
      "pick", "none"
  ...
  old.par <- graphics::par(mar = c(6, 4, 4, 4) + 0.1, xpd = NA, cex = graphics::par()$cex)
  on.exit(graphics::par(old.par))

  if (!is.null(z)) {
    xval <- z$control$xval
    if (xvmult > 1) {
      zresse <- zres <- matrix(NA,
        nrow = nrow(z$cptable),
        ncol = xvmult
      )
      zres[, 1] <- z$cptable[, 4]
    ...
    if (missing(size)) {
      if (xv == "pick") {
        if (xvmult <= 1) {
          ...
          plotcp(z, xvse, pch = 16, col = 2, tab = table(tabmins))
        }
        size.loc <- graphics::locator(1)
        if (!is.null(size.loc)) {
      ...
      if (size <= 2) {
        ...
        cpp <- z$cptable[, 1][min(abs(size - z$cptable[
          ,
          2
        ] - 1)) == abs(size - z$cptable[, 2] - 1)][1]
      }
      z <- prune.rpart(z, cp = cpp)
    }
    if (snip) {
    ...
    if (plot.add) {
      plot.rpart(z,
        uniform = uniform, branch = branch,
        margin = margin
      )
      if (text.add) {
        text.rpart(z,
          digits = digits, xadj = xadj, yadj = yadj,
      ...
      len <- dim(z$cptable)[1]
      foot <- paste("Error : ", signif(
        z$cptable[len, 3],
        digits
      ))
      if (xval[1] > 0) {
        foot <- paste(foot, "  CV Error : ", signif(z$cptable[
          len,
          4
        ], digits), "  SE : ", signif(z$cptable[
          len,
          5
        ], digits))
      }
      graphics::mtext(foot, side = 1, line = 3.5, cex = graphics::par()$cex)
      n <- dim(data)[1]
      if (z$method == "class") {
        nex <- max(table(z$y))
        ...
        graphics::mtext(foot2, side = 1, line = 4.5, cex = graphics::par()$cex)
      }
    }
    if (prn) {
      printcp(z)
    ...
    graphics::text(c(0), c(0), "No splits could be formed", col = 2)
