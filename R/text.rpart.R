...
"text.rpart" <-
  function(
      x, splits = TRUE, which = 4, label = "yval", FUN = graphics::text,
    ...
    parms <- list(
      uniform = uniform, branch = branch, nspace = nspace,
      minbranch = minbranch
    )
    xy <- rpartco(x, parms)
    ...
    if (splits) {
      ...
      if (which == 1) {
        FUN(
          xy$x, xy$y + tadj * cxy[2], rows[left.child],
          ...
        )
      } else {
        if (which == 2 | which == 4) {
    ...
    if (stats) {
      if (is.null(frame$yval2)) {
        ...
        stat <- x$functions$text(
          yval = frame$yval2[leaves, ], dev = frame$dev[leaves], wt = frame$wt[leaves],
          ylevel = ylevels, digits = digits, n = frame$n[leaves],
          use.n = use.n
        )
      }
    ...
    if (bars) {
      ...
      sub.barplot(xy$x, xy$y, bar.vals, leaves,
        xadj = xadj,
        yadj = yadj, bord = bord, line = TRUE, col = c(
          "lightblue",
          "blue", "darkblue"
        )
      )
      ...
      if (legend & !is.null(bar.labs)) {
        graphics::legend(min(xy$x) - 0.1 * rx, max(xy$y) + 0.05 * ry, bar.labs,
